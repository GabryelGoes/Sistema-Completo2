...
export type NfeSefazStatusPayload = {
  certificate_ready: boolean;
  environment: string;
  uf: string | null;
  allow_xml_body_import: boolean;
  missing_env: string[];
  instructions: string[];
};

export type NfeStockInboundItem = {
  item_id: string | null;
  line_number: number;
  ...
  match_method: 'ean' | 'product_code' | 'manual' | null;
  needs_mapping: boolean;
};

export type NfeStockInboundEntry = {
  entry_id: string;
  access_key: string;
  ...
  items: NfeStockInboundItem[];
  items_total: number;
  items_selected: number;
};

export type NfeLookupResponse = {
  source: string;
  sefaz: NfeSefazStatusPayload;
  entry: NfeStockInboundEntry;
};

export type NfeAlreadyImportedError = Error & {
  code: 'NFE_ALREADY_IMPORTED';
  entry?: Record<string, unknown>;
};

export async function getNfeSefazStatus(): Promise<{ ok: boolean; sefaz: NfeSefazStatusPayload }> {
  const response = await fetch(`${API_BASE}/workshop-parts/nfe/status`);
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error(err.error || `Falha ao consultar status NF-e (${response.status})`);
  }
  return response.json();
}

export async function lookupNfeStockInbound(input: {
  access_key: string;
  xml?: string | null;
}): Promise<NfeLookupResponse> {
  const response = await fetch(`${API_BASE}/workshop-parts/nfe/lookup`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      access_key: input.access_key,
      xml: input.xml || undefined,
    }),
  });
  const data = await response.json().catch(() => ({}));
  if (response.status === 409 && data?.code === 'NFE_ALREADY_IMPORTED') {
    const err = new Error(data.error || 'Esta NF-e já foi lançada no estoque.') as NfeAlreadyImportedError;
    err.code = 'NFE_ALREADY_IMPORTED';
    err.entry = data.entry;
    throw err;
  }
  if (!response.ok) {
    const err = new Error(data.error || `Falha ao consultar NF-e (${response.status})`) as Error & {
      code?: string;
      sefaz?: NfeSefazStatusPayload;
      hint?: string;
    };
    err.code = data.code;
    err.sefaz = data.sefaz;
    err.hint = data.hint;
    throw err;
  }
  return data as NfeLookupResponse;
}

export async function mapNfeStockInboundItem(
  entryId: string,
  input: { item_id: string; part_id?: string | null; selected?: boolean }
): Promise<void> {
  const response = await fetch(
    `${API_BASE}/workshop-parts/nfe/${encodeURIComponent(entryId)}/map-item`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(input),
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error(err.error || `Falha ao vincular item (${response.status})`);
  }
}

export async function createPartFromNfeItem(
  entryId: string,
  input: { item_id: string; name?: string; unit_price?: number }
): Promise<WorkshopPart> {
  const response = await fetch(
    `${API_BASE}/workshop-parts/nfe/${encodeURIComponent(entryId)}/create-part`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(input),
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error(err.error || `Falha ao cadastrar produto (${response.status})`);
  }
  const data = (await response.json()) as { part?: Record<string, unknown> } & Record<string, unknown>;
  const row = (data.part ?? data) as Record<string, unknown>;
  return normalizeWorkshopPartRow(row);
...
export async function confirmNfeStockInbound(
  entryId: string,
  input: {
    recorded_by_name?: string | null;
    items: Array<{ item_id: string; part_id: string | null; selected: boolean }>;
  }
): Promise<{ ok: boolean; entry: Record<string, unknown>; result: unknown }> {
  const response = await fetch(
    `${API_BASE}/workshop-parts/nfe/${encodeURIComponent(entryId)}/confirm`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(input),
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Falha ao confirmar entrada (${response.status})`);
  }
  return data;
}
