...
export const BudgetVerifiedSeal: React.FC<BudgetVerifiedSealProps> = ({
  ...
  return (
    <div
      className={`inline-flex max-w-full items-center gap-2 rounded-full border border-emerald-300/70 bg-gradient-to-r from-emerald-50 to-teal-50/90 px-3 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)] ${className}`}
      role="status"
      aria-label="Orçamento verificado"
    >
      ...
      <span className="min-w-0 truncate text-[11px] font-bold uppercase tracking-[0.1em] text-emerald-800">
        Verificado
      </span>
    </div>
  );
