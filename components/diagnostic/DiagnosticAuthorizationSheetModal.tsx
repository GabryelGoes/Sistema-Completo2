...
export const DiagnosticAuthorizationSheetModal: React.FC<DiagnosticAuthorizationSheetModalProps> = ({
  ...
  return (
    <ModalPortal>
      ...
      <div
        className="diag-auth-sheet-backdrop fixed inset-0 z-[200] flex items-center justify-center bg-black/70 p-4 pt-[max(1rem,env(safe-area-inset-top))] pb-[max(1rem,env(safe-area-inset-bottom))] pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] animate-modal-backdrop"
        role="dialog"
        aria-modal="true"
        aria-labelledby="diag-auth-sheet-title"
      >
        <div
          className="diag-auth-sheet-paper relative flex max-h-[calc(100dvh-env(safe-area-inset-top)-env(safe-area-inset-bottom)-2rem)] w-full max-w-2xl min-h-0 flex-col overflow-hidden rounded-lg animate-modal-sheet"
          style={paperModalStyle}
        >
          <div
          ...
          <div className="diag-auth-sheet-no-print relative z-10 flex shrink-0 flex-wrap items-center justify-end gap-3 border-t border-black/10 px-6 py-4">
            <button
              type="button"
              onClick={handlePrint}
