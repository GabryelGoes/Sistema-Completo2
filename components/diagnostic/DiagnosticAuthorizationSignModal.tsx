import React, { useCallback, useEffect, useRef, useState } from "react";
import { PenLine, X, Eraser, Check, Printer, ArrowLeft } from "lucide-react";
import { ModalPortal } from "../ui/ModalPortal";
import { DIAGNOSTIC_AUTHORIZATION_TITLE } from "../../utils/diagnosticAuthorizationTerm";
import { DiagnosticAuthorizationLegalText } from "./DiagnosticAuthorizationLegalText";
import { DiagnosticAuthorizationCertificateDocument } from "./DiagnosticAuthorizationCertificateDocument";
import { DIAGNOSTIC_AUTHORIZATION_PRINT_CSS } from "../../utils/diagnosticAuthorizationPrintCss";

export interface DiagnosticAuthorizationSignModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (blob: Blob, meta: { signaturePreviewDataUrl: string }) => void;
  declarantName?: string;
  vehicleSummary?: string;
  protocolNote?: string | null;
}

type Phase = "sign" | "review";

function setupCanvas(canvas: HTMLCanvasElement) {
  const rect = canvas.getBoundingClientRect();
  const dpr = Math.min(2.5, typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1);
  const w = Math.max(320, Math.floor(rect.width || 320));
  const h = Math.max(160, Math.floor(rect.height || 160));
  canvas.width = Math.floor(w * dpr);
  canvas.height = Math.floor(h * dpr);
  const ctx = canvas.getContext("2d");
  if (!ctx) return null;
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  ctx.scale(dpr, dpr);
  ctx.fillStyle = "#fafafa";
  ctx.fillRect(0, 0, w, h);
  ctx.strokeStyle = "rgba(15,23,42,0.88)";
  ctx.lineWidth = 2.25;
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  return { ctx, cssW: w, cssH: h };
}

export const DiagnosticAuthorizationSignModal: React.FC<DiagnosticAuthorizationSignModalProps> = ({
  open,
  onClose,
  onConfirm,
  declarantName,
  vehicleSummary,
  protocolNote,
}) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const drawingRef = useRef(false);
  const lastRef = useRef<{ x: number; y: number } | null>(null);
  const hasInkRef = useRef(false);
  const pendingBlobRef = useRef<Blob | null>(null);

  const [phase, setPhase] = useState<Phase>("sign");
  const [previewDataUrl, setPreviewDataUrl] = useState<string | null>(null);
  const [issuedAt, setIssuedAt] = useState<Date | null>(null);

  const redrawBase = useCallback(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    setupCanvas(canvas);
    hasInkRef.current = false;
  }, []);

  useEffect(() => {
    if (!open) return;
    setPhase("sign");
    setPreviewDataUrl(null);
    setIssuedAt(null);
    pendingBlobRef.current = null;
    const t = window.setTimeout(() => redrawBase(), 50);
    return () => window.clearTimeout(t);
  }, [open, redrawBase]);

  useEffect(() => {
    if (!open || phase !== "sign") return;
    const t = window.setTimeout(() => redrawBase(), 50);
    return () => window.clearTimeout(t);
  }, [open, phase, redrawBase]);

  useEffect(() => {
    if (!open || phase !== "sign") return;
    const onResize = () => redrawBase();
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, [open, phase, redrawBase]);

  const clientToLocal = (e: React.PointerEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current;
    if (!canvas) return { x: 0, y: 0 };
    const r = canvas.getBoundingClientRect();
    return { x: e.clientX - r.left, y: e.clientY - r.top };
  };

  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    e.preventDefault();
    canvas.setPointerCapture(e.pointerId);
    drawingRef.current = true;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const p = clientToLocal(e);
    lastRef.current = p;
    ctx.beginPath();
    ctx.moveTo(p.x, p.y);
  };

  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    if (!drawingRef.current) return;
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const p = clientToLocal(e);
    const last = lastRef.current;
    if (last) {
      ctx.beginPath();
      ctx.moveTo(last.x, last.y);
      ctx.lineTo(p.x, p.y);
      ctx.stroke();
      hasInkRef.current = true;
    }
    lastRef.current = p;
  };

  const endStroke = (e: React.PointerEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current;
    if (canvas && e.pointerId) {
      try {
        canvas.releasePointerCapture(e.pointerId);
      } catch {
        /* ignore */
      }
    }
    drawingRef.current = false;
    lastRef.current = null;
  };

  const handleClear = () => redrawBase();

  const handleConfirmSignature = () => {
    const canvas = canvasRef.current;
    if (!canvas || !hasInkRef.current) {
      window.alert("Desenhe sua assinatura na área indicada antes de continuar.");
      return;
    }
    canvas.toBlob(
      (blob) => {
        if (!blob) {
          window.alert("Não foi possível gerar a imagem da assinatura. Tente novamente.");
          return;
        }
        pendingBlobRef.current = blob;
        const reader = new FileReader();
        reader.onloadend = () => {
          const url = typeof reader.result === "string" ? reader.result : null;
          if (!url) {
            window.alert("Não foi possível preparar o documento para visualização.");
            return;
          }
          setPreviewDataUrl(url);
          setIssuedAt(new Date());
          setPhase("review");
        };
        reader.readAsDataURL(blob);
      },
      "image/png",
      0.92
    );
  };

  const handleBackToSign = () => {
    if (previewDataUrl?.startsWith("blob:")) {
      try {
        URL.revokeObjectURL(previewDataUrl);
      } catch {
        /* ignore */
      }
    }
    setPreviewDataUrl(null);
    pendingBlobRef.current = null;
    setIssuedAt(null);
    setPhase("sign");
    window.setTimeout(() => redrawBase(), 80);
  };

  const handlePrint = () => {
    window.print();
  };

  const handleFinish = () => {
    const blob = pendingBlobRef.current;
    const url = previewDataUrl;
    if (!blob || !url) return;
    onConfirm(blob, { signaturePreviewDataUrl: url });
    pendingBlobRef.current = null;
    setPreviewDataUrl(null);
    setIssuedAt(null);
    setPhase("sign");
    onClose();
  };

  const handleRequestClose = () => {
    if (previewDataUrl && previewDataUrl.startsWith("blob:")) {
      try {
        URL.revokeObjectURL(previewDataUrl);
      } catch {
        /* ignore */
      }
    }
    setPreviewDataUrl(null);
    pendingBlobRef.current = null;
    setIssuedAt(null);
    setPhase("sign");
    onClose();
  };

  if (!open) return null;

  return (
    <ModalPortal>
      <style dangerouslySetInnerHTML={{ __html: DIAGNOSTIC_AUTHORIZATION_PRINT_CSS }} />

      {phase === "sign" ? (
        <div
          className="fixed inset-0 z-[240] flex items-end justify-center bg-black/55 p-0 pt-10 backdrop-blur-md sm:items-center sm:p-6 sm:pt-[max(1rem,env(safe-area-inset-top))] sm:pb-[max(1rem,env(safe-area-inset-bottom))]"
          role="dialog"
          aria-modal="true"
          aria-labelledby="diag-auth-modal-title"
        >
          <div
            className="flex max-h-[min(94dvh,960px)] w-full max-w-2xl flex-col overflow-hidden rounded-t-[28px] border border-zinc-200/90 bg-white shadow-[0_-12px_48px_-16px_rgba(0,0,0,0.35)] animate-in slide-in-from-bottom-4 duration-300 dark:border-white/[0.1] dark:bg-zinc-950 sm:max-h-[min(90vh,940px)] sm:rounded-[28px] sm:shadow-2xl"
            onClick={(ev) => ev.stopPropagation()}
          >
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-zinc-200/80 px-5 py-4 dark:border-white/[0.08]">
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-[#007AFF]/25 bg-[#007AFF]/[0.1] dark:border-[#007AFF]/35 dark:bg-[#007AFF]/15">
                  <PenLine className="h-5 w-5 text-[#007AFF] dark:text-[#93c5fd]" strokeWidth={2.25} aria-hidden />
                </div>
                <div className="min-w-0">
                  <h2
                    id="diag-auth-modal-title"
                    className="text-[15px] font-bold leading-tight tracking-tight text-zinc-900 dark:text-white sm:text-[16px]"
                  >
                    {DIAGNOSTIC_AUTHORIZATION_TITLE}
                  </h2>
                  <p className="mt-0.5 text-[12px] font-medium text-zinc-500 dark:text-zinc-400">
                    Leia as cláusulas e assine na área indicada. Em seguida você poderá imprimir ou salvar em PDF.
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={handleRequestClose}
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-zinc-100 text-zinc-600 transition-colors hover:bg-zinc-200 dark:bg-white/[0.08] dark:text-zinc-300 dark:hover:bg-white/[0.12]"
                aria-label="Fechar"
              >
                <X className="h-5 w-5" />
              </button>
            </div>

            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-4 [-webkit-overflow-scrolling:touch]">
              <div className="rounded-2xl border border-zinc-200/80 bg-zinc-50/90 px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] dark:border-white/[0.08] dark:bg-zinc-900/40 sm:px-5 sm:py-5">
                <DiagnosticAuthorizationLegalText
                  presentation="modal"
                  declarantName={declarantName}
                  vehicleSummary={vehicleSummary}
                />
              </div>

              <p className="mt-6 text-[11px] font-bold uppercase tracking-[0.16em] text-zinc-500 dark:text-zinc-400">
                Assinatura do declarante (campo obrigatório)
              </p>
              <div className="mt-2 overflow-hidden rounded-2xl border-2 border-dashed border-zinc-300/95 bg-white dark:border-white/[0.14] dark:bg-zinc-900/30">
                <canvas
                  ref={canvasRef}
                  className="touch-none block h-[min(200px,30vh)] w-full cursor-crosshair select-none"
                  onPointerDown={handlePointerDown}
                  onPointerMove={handlePointerMove}
                  onPointerUp={endStroke}
                  onPointerCancel={endStroke}
                  onPointerLeave={(e) => {
                    if (drawingRef.current) endStroke(e);
                  }}
                />
              </div>
            </div>

            <div className="flex shrink-0 flex-col gap-2 border-t border-zinc-200/80 bg-white/95 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] dark:border-white/[0.08] dark:bg-zinc-950/95 sm:flex-row sm:flex-wrap sm:justify-end">
              <button
                type="button"
                onClick={handleClear}
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-zinc-200/90 bg-zinc-50 px-4 text-[14px] font-semibold text-zinc-800 transition-colors hover:bg-zinc-100 active:scale-[0.99] dark:border-white/[0.12] dark:bg-white/[0.06] dark:text-zinc-100 dark:hover:bg-white/[0.1] sm:order-1 sm:h-11"
              >
                <Eraser className="h-4 w-4 shrink-0" aria-hidden />
                Limpar
              </button>
              <button
                type="button"
                onClick={handleRequestClose}
                className="inline-flex h-12 items-center justify-center rounded-2xl border border-zinc-200/90 px-4 text-[14px] font-semibold text-zinc-600 transition-colors hover:bg-zinc-50 active:scale-[0.99] dark:border-white/[0.12] dark:text-zinc-300 dark:hover:bg-white/[0.06] sm:order-2 sm:h-11"
              >
                Cancelar
              </button>
              <button
                type="button"
                onClick={handleConfirmSignature}
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-[#007AFF] px-5 text-[14px] font-semibold text-white shadow-lg shadow-blue-500/25 transition-all hover:opacity-95 active:scale-[0.98] sm:order-3 sm:h-11"
              >
                <Check className="h-4 w-4 shrink-0" strokeWidth={2.5} aria-hidden />
                Gerar documento
              </button>
            </div>
          </div>
        </div>
      ) : (
        <div
          className="diag-auth-cert-backdrop fixed inset-0 z-[240] flex flex-col bg-zinc-900/80 backdrop-blur-md print:bg-white"
          role="dialog"
          aria-modal="true"
          aria-labelledby="diag-auth-review-title"
        >
          <div className="diag-auth-cert-no-print flex shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-zinc-950/90 px-4 py-3 sm:px-5">
            <h2 id="diag-auth-review-title" className="min-w-0 text-[15px] font-bold text-white sm:text-[16px]">
              Documento para conferência, impressão ou PDF
            </h2>
            <button
              type="button"
              onClick={handleRequestClose}
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20"
              aria-label="Fechar sem concluir"
            >
              <X className="h-5 w-5" />
            </button>
          </div>

          <div className="min-h-0 flex-1 overflow-y-auto bg-zinc-300/90 p-3 sm:p-6 print:bg-white print:p-0">
            {previewDataUrl ? (
              <DiagnosticAuthorizationCertificateDocument
                signatureDataUrl={previewDataUrl}
                declarantName={declarantName}
                vehicleSummary={vehicleSummary}
                protocolNote={protocolNote}
                issuedAt={issuedAt ?? new Date()}
              />
            ) : null}
          </div>

          <div className="diag-auth-cert-no-print border-t border-white/10 bg-zinc-950/95 p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
            <div className="mx-auto flex max-w-3xl flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-end">
              <button
                type="button"
                onClick={handleBackToSign}
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-white/20 px-4 text-[14px] font-semibold text-white transition-colors hover:bg-white/10 sm:h-11"
              >
                <ArrowLeft className="h-4 w-4 shrink-0" aria-hidden />
                Voltar à assinatura
              </button>
              <button
                type="button"
                onClick={handlePrint}
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-white px-5 text-[14px] font-semibold text-zinc-900 shadow-md transition-opacity hover:opacity-95 sm:h-11"
              >
                <Printer className="h-4 w-4 shrink-0" aria-hidden />
                Imprimir ou salvar PDF
              </button>
              <button
                type="button"
                onClick={handleFinish}
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-[#007AFF] px-5 text-[14px] font-semibold text-white shadow-lg shadow-blue-500/25 transition-all hover:opacity-95 active:scale-[0.98] sm:h-11"
              >
                <Check className="h-4 w-4 shrink-0" strokeWidth={2.5} aria-hidden />
                Concluir e usar na ficha
              </button>
            </div>
          </div>
        </div>
      )}
    </ModalPortal>
  );
};
