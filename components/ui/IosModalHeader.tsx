...
export const IosModalHeader: React.FC<IosModalHeaderProps> = ({ icon, title, subtitle }) => (
  <div className="flex items-center gap-3 mb-1 pr-2">
    <div className={iosAccentIconShellModal}>
      ...
      <div className="relative z-10 flex h-full w-full min-h-0 min-w-0 items-center justify-center overflow-hidden rounded-[inherit] text-zinc-950 dark:text-white [&_img]:h-full [&_img]:w-full [&_img]:min-h-0 [&_img]:min-w-0 [&_img]:object-cover [&_svg]:h-6 [&_svg]:w-6 [&_path]:fill-current [&_svg]:[filter:drop-shadow(0_1px_0_rgba(255,255,255,0.45))] dark:[&_svg]:[filter:drop-shadow(0_1px_2px_rgba(0,0,0,0.5))]">
        {icon}
      </div>
    </div>
