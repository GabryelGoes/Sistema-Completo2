...
export function createApiApp() {
  ...
  /** Lista avisos do quadro de avisos da oficina, usados no app e na TV. */
  app.get("/api/notices", async (_req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin
        ...
        .select("id, title, body, highlight, active, sort_order, created_at, updated_at")
        .eq("workshop_id", WORKSHOP_ID)
        .order("sort_order", { ascending: true, nullsFirst: true })
        .order("created_at", { ascending: true });
      if (error) {
        console.error("[API] Erro em GET /api/notices:", error);
      ...
      console.error("[API] Erro em GET /api/notices:", err);
      return res.status(500).json({ error: err?.message ?? "Erro desconhecido" });
    }
  });

  /** Cria novo aviso para o quadro de avisos. */
  app.post("/api/notices", async (req, res) => {
    try {
      if (!supabaseAdmin || !WORKSHOP_ID) {
        return res.status(500).json({ error: "Supabase não configurado." });
      }
      const { title, body, highlight, active, sortOrder } = req.body || {};
      const t = typeof title === "string" ? title.trim() : "";
      const b = typeof body === "string" ? body.trim() : "";
      if (!t) return res.status(400).json({ error: "Informe o título do aviso." });
      if (!b) return res.status(400).json({ error: "Informe o texto do aviso." });
      ...
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          highlight: !!highlight,
          active: active !== false,
          sort_order: typeof sortOrder === "number" ? sortOrder : null,
        })
        .select("id, title, body, highlight, active, sort_order, created_at, updated_at")
        .single();
      if (error) {
        console.error("[API] Erro em POST /api/notices:", error);
        return res.status(500).json({ error: error.message });
      }
      return res.json({
        id: data.id,
      ...
      console.error("[API] Erro em POST /api/notices:", err);
      return res.status(500).json({ error: err?.message ?? "Erro desconhecido" });
    }
  });
  ...
  app.patch("/api/notices/:id", async (req, res) => {
    try {
      ...
      if (sortOrder !== undefined) patch.sort_order = sortOrder === null ? null : Number(sortOrder);
      ...
      const { data, error } = await supabaseAdmin
        ...
        .single();
      if (error) {
        console.error("[API] Erro em PATCH /api/notices/:id:", error);
        return res.status(500).json({ error: error.message });
      }
      return res.json({
        id: data.id,
      ...
      console.error("[API] Erro em PATCH /api/notices/:id:", err);
      return res.status(500).json({ error: err?.message ?? "Erro desconhecido" });
    }
  });
  ...
  app.delete("/api/notices/:id", async (req, res) => {
    try {
      ...
      const { error } = await supabaseAdmin
        ...
        .eq("workshop_id", WORKSHOP_ID);
      if (error) {
        console.error("[API] Erro em DELETE /api/notices/:id:", error);
        return res.status(500).json({ error: error.message });
      }
      return res.status(204).end();
    } catch (err: any) {
      console.error("[API] Erro em DELETE /api/notices/:id:", err);
      return res.status(500).json({ error: err?.message ?? "Erro desconhecido" });
    }
  });

  // ---------- Meta semanal (para quadro de avisos / TV) ----------

  /** Retorna a meta semanal atual, criando uma semana padrão se não existir. */
  app.get("/api/weekly-goal", async (_req, res) => {
    try {
      if (!supabaseAdmin || !WORKSHOP_ID) {
        return res.status(404).json({ error: "Meta semanal não configurada." });
      }
      ...
      const { data, error } = await supabaseAdmin
        .from("workshop_weekly_goals")
        .select("id, week_start, target_amount, current_amount, created_at, updated_at")
        .eq("workshop_id", WORKSHOP_ID)
        .eq("week_start", weekStart)
        .maybeSingle();
      if (error) {
        console.error("[API] Erro em GET /api/weekly-goal:", error);
        return res.status(500).json({ error: error.message });
      }
      if (!data) {
        return res.status(404).json({ error: "Nenhuma meta definida para esta semana." });
      }
      return res.json({
        id: data.id,
      ...
      console.error("[API] Erro em GET /api/weekly-goal:", err);
      return res.status(500).json({ error: err?.message ?? "Erro desconhecido" });
    }
  });

  /** Atualiza/define a meta semanal e soma faturamento recebido. */
  app.patch("/api/weekly-goal", async (req, res) => {
    try {
      if (!supabaseAdmin || !WORKSHOP_ID) {
        return res.status(500).json({ error: "Supabase não configurado." });
      }
      const { targetAmount, addAmount } = req.body || {};
      ...
      const { data, error } = await supabaseAdmin
        ...
        .maybeSingle();
      if (error) {
        console.error("[API] Erro em PATCH /api/weekly-goal (select):", error);
        return res.status(500).json({ error: error.message });
      }

      const now = new Date().toISOString();
      if (!data) {
        const initialTarget = typeof targetAmount === "number" && targetAmount > 0 ? targetAmount : 0;
        const initialCurrent = typeof addAmount === "number" ? Math.max(0, addAmount) : 0;
        const { data: inserted, error: insErr } = await supabaseAdmin
          .from("workshop_weekly_goals")
          .insert({
            workshop_id: WORKSHOP_ID,
            week_start: weekStart,
            target_amount: initialTarget,
          ...
          .single();
        if (insErr) {
          console.error("[API] Erro em PATCH /api/weekly-goal (insert):", insErr);
          return res.status(500).json({ error: insErr.message });
        }
        return res.json({
          id: inserted.id,
          weekStart: inserted.week_start,
          targetAmount: Number(inserted.target_amount ?? 0),
          currentAmount: Number(inserted.current_amount ?? 0),
          createdAt: inserted.created_at,
          updatedAt: inserted.updated_at,
        });
      }

      let newTarget = Number(data.target_amount ?? 0);
      let newCurrent = Number(data.current_amount ?? 0);
      if (typeof targetAmount === "number" && targetAmount > 0) {
        newTarget = targetAmount;
      }
      if (typeof addAmount === "number" && addAmount !== 0) {
        newCurrent = Math.max(0, newCurrent + addAmount);
      }
      ...
      const { data: updated, error: updErr } = await supabaseAdmin
        ...
        .single();
      if (updErr) {
        console.error("[API] Erro em PATCH /api/weekly-goal (update):", updErr);
        return res.status(500).json({ error: updErr.message });
      }
      return res.json({
        id: updated.id,
        weekStart: updated.week_start,
        targetAmount: Number(updated.target_amount ?? 0),
        currentAmount: Number(updated.current_amount ?? 0),
        createdAt: updated.created_at,
        updatedAt: updated.updated_at,
      });
    } catch (err: any) {
      console.error("[API] Erro em PATCH /api/weekly-goal:", err);
      return res.status(500).json({ error: err?.message ?? "Erro desconhecido" });
    }
  });
